...
phsnm = ["map","oak","plot","CLM","Noah","SiB"]
soilm = [2,10]
...
for i in range(len(phsnm)):
    for n in range(len(soilm)):
        for m in range(len(month)):
            ...
            # Q 
            Q = np.ma.zeros([1, 400, 4, 700])

            # GPP
            GPP = np.ma.zeros([1, 400, 700])
            ...
            for y in range(1980,2018):
                ...
                VQS2R = nc.Dataset(input_file).variables['VQS2R'][:]
                PSN = nc.Dataset(input_file).variables['PSN'][:]
